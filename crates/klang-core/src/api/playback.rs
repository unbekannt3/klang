...
pub async fn play_tidal_track(
    ...
    let (stream_info, uri, norm_gain, rg, peak, _is_dash) =
        resolve_play_uri(state, track_id, use_track_gain).await?;
...
pub fn clear_next_track(state: &AppState) -> Result<(), SoneError> {
...
pub async fn pause_track(state: &AppState) -> Result<(), SoneError> {
...
pub async fn resume_track(state: &AppState) -> Result<(), SoneError> {
...
pub async fn stop_track(state: &AppState) -> Result<(), SoneError> {
    log::debug!("[stop_track]");
    stop_playback(state).await
}

pub fn set_volume(state: &AppState, level: f32) -> Result<(), SoneError> {
    state
        .audio_player
        .set_volume(level)
        .map_err(SoneError::Audio)?;
...
pub fn get_playback_position(state: &AppState) -> Result<f32, SoneError> {
    state.audio_player.get_position().map_err(SoneError::Audio)
}

pub async fn seek_track(state: &AppState, position_secs: f32) -> Result<(), SoneError> {
...
pub fn is_track_finished(state: &AppState) -> Result<bool, SoneError> {
...
pub fn load_playback_queue(state: &AppState) -> Result<Option<String>, SoneError> {
