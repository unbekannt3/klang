...
pub async fn load_saved_auth(state: &AppState) -> Result<Option<AuthTokens>, SoneError> {
...
pub fn get_saved_credentials(state: &AppState) -> Result<(String, String), SoneError> {
...
pub async fn refresh_tidal_auth(state: &AppState) -> Result<AuthTokens, SoneError> {
...
pub async fn logout(state: &AppState) -> Result<(), SoneError> {
    log::debug!("[logout]");
    ...
    crate::api::playback::stop_playback(state)
        .await
        .ok();
...
pub fn consume_legacy_auth_notice(state: &AppState) -> Result<bool, SoneError> {
...
pub async fn get_session_user_id(state: &AppState) -> Result<u64, SoneError> {
...
// custom-credentials path. The functions below use the embedded PKCE
// credentials (`stream_key_c/d`) and never expose them to the caller.
//
// Upstream also drove PKCE through an embedded Tauri WebView window, catching
// the redirect in a navigation handler. That flow is gone: the browser variant
// (`start_pkce_browser_login` / `complete_pkce_browser_login`) opens the user's
// own browser and takes the redirect URL back, which needs no WebView at all.
